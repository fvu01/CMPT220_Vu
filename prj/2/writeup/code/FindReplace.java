...
public class FindReplace extends JDialog implements ActionListener {
  ...
  public FindReplace(JFrame owner, boolean isReplace) {
    super(owner, true);
  ...
  private void process() {
    ...
    int caret = TypeIt.getArea().getCaretPosition();
    String word = getWord();
  ...
  private void endResult(boolean isReplaceAll, int tally) {
    ...
    if(isReplaceAll) {
      ...
      if(foundOne && !isReplace) {
        ...
        message = "End of Replace" + searchText.getText() + " with " + 
        replaceText.getText();
      }
    }
  ...
  private int search(String text, String word, int caret) {
    ...
    TypeIt.getArea().setCaretPosition(0);
    
    if(found) {
      TypeIt.getArea().requestFocus();
      if(isScarchDown()) {
        TypeIt.getArea().select(caret, caret+check);
      }
      else {
        TypeIt.getArea().select(caret-check, caret);
      }
      //for replace
      if(isReplace) {
        String replace = replaceText.getText();
        TypeIt.getArea().replaceSelection(replace);
        if(isScarchDown()) {
          TypeIt.getArea().select(caret, caret+replace.length());
        }
        else {
          TypeIt.getArea().select(caret-replace.length(), caret);
        }
      }
      foundOne = true;
      return caret;
  ...
  private String getAllText() {
    if(caseNotSelected()) {
      return TypeIt.getArea().getText().toLowerCase();
    }
    return TypeIt.getArea().getText();
  }
  ...
  private void replaceAll() {
    ...
    String text = TypeIt.getArea().getText();
    ...
    TypeIt.getArea().setText(sb.toString());
